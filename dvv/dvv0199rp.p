...
/*****************************************************************************
    ...
    data: 07/12/2021 - Frank. Alterado a busca da tabela emitente para buscar 
                              pelo codigo quando nÆo encontrar pelo nome-abrev.
*****************************************************************************/
...
{dvv/dvv0199.i} /* Defini‡äes Fun‡äes */
...
PROCEDURE pi-cabecalho:
    ...
    */
    ...
    excelAppl:Range(fi-nc() + string(i-lin)):Value = "Transa‡Æo APB".
    ...
    excelAppl:Range(fi-nc() + string(i-lin)):Value = "RegiÆo".
    ...
    excelAppl:Range(fi-nc() + string(i-lin)):Value = "Descri‡Æo".
...
/*                     END.                                                                                     */
            ...
            /* Caso nÆo encontre despesa imprime o valor e chave da medi‡Æo. */
    ...
    for each tt-movto-cep where
        ...
        **/
        ...
        IF AVAIL item-doc-est THEN DO:
		    ...
		    //aqui testar...senão retirar estava na versão do Carlos.
                    //PUT "e" SKIP.
        ...
        IF CAN-FIND(FIRST tt-acum-medicao) THEN DO:
            ...
            for each tt-acum-medicao:
                ...
                /* Calcula o rateio das despesas em cima do valor da medi‡Æo, controlando o arredondamento. */
                ...
                /******************
                //colocado de forma fixa pois esta dando um erro na gera‡Æo deste lote e esta transa‡Æo, esta considerando em duas realiza‡äes.
...
/*                                                                                                   */
/*                     /* Esta tabela dvv_tit_ap come‡ar  a ter registros a partir de 01/08/2013. */ */
/*                                                                                                   */
/*                     for FIRST dvv_tit_ap no-lock where                                            */
/*                               dvv_tit_ap.cod_estab       = b_tit_ap.cod_estab                     */
        ...
        FOR EACH tt-conta-contab:
            ...
            IF ITEM.tipo-contr = 2 THEN DO: /* Carrega Valor somente para Itens com Controle Total */
                if  v_val_movto = 0 then do:
                    /*-----> Busca éltimo Valor do Material qdo o Movimento for Zero <----------------------*/
...
PROCEDURE pi-cria-dados-dvv:
    ...
    assign b-tt-rel-reg.tp-origem    = dvv_movto_det.tp_doc_origem
         ...
         //  b-tt-rel-reg.dt-conf      = dvv_movto_det.data_movto_real  - NT Carlos Jr - retirada esta regra por solicita‡Æo do Rios 21.05.24 chamado 0524-003623
         .
...
/*           b-tt-rel-reg.vl-prev     = IF v-val-desp[1] <> 0 THEN v-val-desp[1] ELSE (IF AVAIL dvv_movto THEN v_val_prev_R$ ELSE 0)*/
    ...
    /*
    ** PrevisÆo Trading ‚ em outro estabelecimento
    */
...
/* 1. Busca as contas cont beis que serÆo utilizados no relat¢rio de acordo com a faixa parametrizada em tela. */
    ...
    /* 2. Busca as movimenta‡äes do recebimento na tabela de movimento de estoque.
          Estas informa‡äes nÆo sÆo contabilzadas detalhadamente. */
    run pi-busca-dados-estoque.
    
    /*run pi-busca-dados-tms.*/    
    
    /* --- Encontrar cotacao entre Indic Base de Indic Apresenta‡Æo ---*/
    ...
    /* 3. Busca as movimenta‡äes das contas cont beis. */
    FOR EACH tt-cta-ctbl
        ...
        FOR EACH ITEM_lancto_ctbl NO-LOCK WHERE
            ...
            /* O relat½rio deverÿ sempre apresentar os valores em reais.
               Caso o indicador econËmico seja diferente de Real buscar na apropria»’o o valor em reais. */
            IF ITEM_lancto_ctbl.cod_indic_econ <> cRealVenda THEN DO:
    ...
    FOR EACH tt-rel
        ...
        IF tt-rel.nr-nota-fis <> "" THEN DO:
            ...
            IF AVAIL nota-fiscal THEN DO:
                ...
                //alterado o codigo para buscar o emitente pelo cod-emitente quando nÆo encontrar pelo nome-abrev
                //tivemos casos onde o nome-abrev estava com acento e no cd0704 estava sem e nÆo encontrava.
                FIND emitente WHERE
                ...
                ASSIGN tt-rel.cod-emitente = emitente.cod-emitente
                       ...
                       //aqui tt-rel.dt-conf      = nota-fiscal.dt-emis-nota - NT Carlos Jr - retirada esta regra por solicita‡Æo do Rios 21.05.24 chamado 0524-003623
                       .                   
...
PROCEDURE pi-rateio-despesa:
    ...
    for each tt-rel
        ...
        IF tt-rel.movto = "realizado" OR tt-rel.movto = "previsto" THEN DO:
            ...
            IF tt-rel.tp-origem = 1 /*"Medi‡Æo"*/ THEN DO:
    ...
    IF tt-param.l-despesa-nao-rast = YES THEN DO:
        ...
        DO v-dat = tt-param.dt-ctbl-ini TO tt-param.dt-ctbl-fim:
            ...
            FOR EACH dvv_contabil NO-LOCK
                ...
                ASSIGN tt-rel-item.MODULO       = "NÆo Rast"
                       tt-rel-item.val-lancto   = tt-rel-item.vl-real 
                       tt-rel-item.val-doc      = tt-rel-item.vl-real
                       tt-rel-item.cod-depos    = dvv_contabil.char-1
...
PROCEDURE pi-monta-fech-previo:
    ...
    RUN pi-acompanhar IN h-acomp (INPUT "Buscando dados FECHAMENTO PRVIO.").
    FIND plano_cta_ctbl NO-LOCK WHERE
         plano_cta_ctbl.cod_plano_cta_ctbl      = "nitro"
     AND plano_cta_ctbl.ind_tip_plano_cta_ctbl = "Prim rio"
     AND plano_cta_ctbl.dat_inic_valid         <= TODAY
                ...
                DO v-dat = tt-param.dt-ctbl-ini TO tt-param.dt-ctbl-fim:
                    ...
                    FOR EACH aprop_ctbl_ap NO-LOCK WHERE
                       ...
                       FIRST cta_ctbl NO-LOCK WHERE
                             ...
                             cta_ctbl.ind_espec_cta_ctbl      <> "Sint‚tica",
                    ...
                    FOR EACH movto-estoq NO-LOCK USE-INDEX estab-dep WHERE 
                       ...
                       /********** Gera RazÆo para a conta MOVIMENTO */
                       ...
                       /********** Gera RazÆo para a conta COFINS */
                    ...
                    FOR EACH item_lancto_ctbl   NO-LOCK USE-INDEX tmlnctcb_estab WHERE
                        ...
                        /* Filtra movimentos do modulo de EXPORTA€ÇO
                ...
                for each dvv_tit_ap no-lock where
                    ...
                    if dvv_tit_ap.classif = "DVV" then do:
                        ...
                        if dvv_tit_ap.classif = "DEX" then do:
                            ...
                            IF NOT AVAIL dex_movto_det THEN DO: //caso nÆo encontre o dex_movto_det
                                for first dex_movto no-lock where
                                          dex_movto.cod_estabel   = dvv_tit_ap.cod_estab
                                      AND dex_movto.nr_pedido     = dvv_tit_ap.nr_proc_exp
                                      and dex_movto.nr_processo   = dvv_tit_ap.nr_proc_exp
    ...
    IF AVAIL tt-movto-cep THEN do: 
        ...
        */
        ...
        ELSE DO:
            ...
            IF NOT AVAIL dvv_movto_comis_rep THEN DO: //para nÆo criar em duplicidade as informa‡äes quando forem de origem comissos
                ASSIGN v-val-docto              = tt-movto-cep.val_lancto_ctbl.
...
PROCEDURE pi_busca_lote_lancto_item:
    ...
    */
    ...
    /* PRIMEIRO procura informa»„es na TEMP-TABLE (Melhoria de Performance) */
    ...
    /* SEGUNDO: Procura informa»„es no ITEM_LANCTO_CTBL */
